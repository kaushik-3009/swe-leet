...
**The AI grader is a hard dependency-injected fallback, not a hard requirement.** `gradeWithAi()` returns `null` (not a thrown error) if `ANTHROPIC_API_KEY` is unset or the API call fails for any reason, and `gradeSubmission()` falls back to `structural.coverage` alone with auto-generated feedback strings. This was validated directly: `src/lib/grading/ai.test.ts` asserts `gradeWithAi()` returns `null` with no key set, and `index.test.ts` asserts the blended-vs-fallback score math in both branches.
...
**~15 problems per track rather than "15-20" exactly (29 total: 15 System Design, 14 LLD).** Chosen to keep every problem's description, rubric, and explanation genuinely substantive (interview-realistic requirements, specific non-obvious trade-off callouts in the reference explanation) rather than padding the count with filler entries — the schema and seed script scale to hundreds of problems without any changes, so growing the catalog later is just adding more `ProblemSpec` entries.
