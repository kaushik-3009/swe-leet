...
export default function EntryList({ refreshKey, userId, showViewAll, scrollable = true }: Props) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-base font-semibold flex items-center gap-2" style={{ fontFamily: "var(--font-display)", color: "var(--text-primary)" }}>
      ...
      {entries.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center flex-1 py-12 gap-3.5">
          <div className="w-[52px] h-[52px] rounded-xl flex items-center justify-center" style={{ background: "var(--card-elevated)", border: "1px solid var(--border-strong)", color: "var(--text-tertiary)" }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 11l3 3L22 4M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>
          </div>
          <div className="font-semibold text-[15px]" style={{ color: "var(--text-primary)" }}>No entries yet</div>
          <div className="text-[13px] max-w-[320px]" style={{ color: "var(--text-tertiary)" }}>Log your first study session on the left to start building your streak.</div>
        </div>
      ) : scrollable ? (
